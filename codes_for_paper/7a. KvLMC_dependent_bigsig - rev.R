...
##### x is binary and z is also binary, repeated measures #####


rm(list = ls())

plotdir <- paste0(getwd(), "/plots/")
resultdir <- paste0(getwd(), "/results/KVLMC_dependent_bigsig - rev/")
...
set.seed(1)
seedmat <- matrix(sample(1e6, 10000, replace = F), ncol = 2)

out <- simulation_study_4(betarev1, betaX, Xmat, nvec, sigb,
                          betaZ = NULL, Zmat, 
                          subjind, sig.subj,
                          num.sim = 5000, num.perm = 10000, 
