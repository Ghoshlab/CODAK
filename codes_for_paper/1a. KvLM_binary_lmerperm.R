...
######## Binary predictor + LRT-permutation #########


rm(list = ls())

plotdir <- paste0(getwd(), "/plots/")
resultdir <- paste0(getwd(), "/results/KVLM_binary/")
...
out <- simulation_study_2(betarev1, betaX, Xmat, nvec, sigb,
                          num.sim = 100, num.perm = 100000, 
                          whichtests = c("LogisticM"),
                          lmertest = c("Permutation"), lmer.nump = 500,
                          printafter = 5, 
                          storeY = F)


save(out, probs, probs.new, file = paste0(resultdir, "no_diff_lmerperm.RObj"))
...
ds <- numeric(rep)
pval.logistic.mixed.perm <- matrix(NA, nrow = rep, ncol = num.sim)
...
for (i in 1:rep){
  ...
  pval.logistic.mixed.perm[i, ] <- out$logistic.mixed[,1]
}

save(pval.logistic.mixed.perm,
     ors, ds, probs, probs.new,
     file = paste0(resultdir, "small_diff_all_lmerperm.RObj"))
...
save(pval.logistic.mixed.perm,
     ors, ds, probs, probs.new,
     file = paste0(resultdir, "small_diff_some_lmerperm.RObj"))
...
save(pval.logistic.mixed.perm,
     ors, ds, probs, probs.new,
     file = paste0(resultdir, "large_diff_few_lmerperm.RObj"))
