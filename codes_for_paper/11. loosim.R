...
resultdir <- paste0(getwd(), "/results/LOO/")
...
for (cases in 1:3){
  ...
  for (i in 1:rep){
    ...
    for (j in 1:num.sim){
      ...
      out <- try(LOO.comp(propmat, x), silent = T)
      ...
      out <- try(dcor.comp.w(propmat, x, result = "cor",
                              wt.in = NULL), silent = T)
      if (class(out) == "try-error"){
        wts.store[i, j, ] <- NA
      }else{
...
load(paste0(resultdir, "LOOsim_final.RObj"))
...
Cairo(file = paste0(plotdir, "LOO_histogram.pdf"), typ = "pdf", dpi = 95,
      height = 500, width = 1300, pointsize = 16)
par(mfrow = c(1,3))
barplot(table(top.detect)/sum(table(top.detect)), ylab = "Relative Frequency", ylim = c(0, 0.45),
        xlab = "Number detected (of top 5)", main = "dcor_LOO", col = "firebrick3")
barplot(table(top.detect.w)/sum(table(top.detect.w)), ylab = "Relative Frequency", ylim = c(0, 0.45),
        xlab = "Number detected (of top 5)", main = "dcor_weighted", col = "lightcoral")
barplot(table(top.detect.or_v_obsor)/sum(table(top.detect.or_v_obsor)), ylab = "Relative Frequency",
        ylim = c(0, 0.45),
        xlab = "Number detected (of top 5)", main = "Odds ratio (true model)", col = "royalblue2")
dev.off()
