...
resultdir <- paste0(getwd(), "/results/KVLM_binary_bigsig - rev/")
...
out <- simulation_study_2(betarev1, betaX, Xmat, nvec, sigb,
                          ...
                          whichtests = c("Kernel", "Euclidean", 
                                         "dcor", "mirkat", "Kernel.BC"),
...
ds <- numeric(rep)
pval.kernel <- pval.kernel.bc <- pval.logistic.mixed.wald <- pval.logistic.mixed.lrt <- 
...
for (i in 1:rep){
  ...
  out <- simulation_study_2(betarev1, betaX, Xmat, nvec, sigb, 
                            ...
                            whichtests = c("Kernel", "Kernel.BC","dcor", "Euclidean", 
                                           "dcor", "mirkat"),
                            diffcyt.method = c("edgeR", "voom"),
                            mirkat.method = c("Opt"),
                            printafter = 10)
...
save(pval.kernel, pval.kernel.bc, pval.dcor, pval.euclidean, 
     pval.mirkat.AD, pval.mirkat.BC, pval.mirkat.opt,
