...
##### x is continuous and z is binary #####


rm(list = ls())

plotdir <- paste0(getwd(), "/plots/")
resultdir <- paste0(getwd(), "/results/KVLMC_xcont_zbin/")
...
pval.kernel.sk <- pval.kernel.alr <- pval.kernel.clr <-
  pval.logistic.mixed.wald <- pval.logistic.mixed.lrt <- matrix(NA, nrow = rep, ncol = num.sim)
