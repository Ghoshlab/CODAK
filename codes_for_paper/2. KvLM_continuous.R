...
resultdir <- paste0(getwd(), "/results/KVLM_continuous/")
...
ds <- numeric(rep)
pval.kernel <- pval.logistic.mixed.wald <- pval.logistic.mixed.lrt <- matrix(NA, nrow = rep, ncol = num.sim)
...
for (i in 1:rep){
  ...
  betaX <- sqrt(3)*as.matrix((betarev2 - betarev1)) 
  #To maintain similar variance as the binary case (old sims had 1.5 multiplier)
